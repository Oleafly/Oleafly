use crate::{process, Engine, Error, ErrorKind, Result, Workspace};
use serde::Serialize;
...
impl Workspace {
    pub fn prepare_build(&self) -> Result<PreparedBuild> {
        let main_document = self.manifest().main_doc.clone();
        Ok(PreparedBuild {
...
fn latexmk_arguments(build: &PreparedBuild, options: BuildOptions) -> Result<Vec<OsString>> {
    let flavor = build.tex_flavor.as_deref().map_or_else(
