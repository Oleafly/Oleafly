...
OpenLeaf can act as an MCP (Model Context Protocol) server. Any MCP client (Claude Desktop, Claude Code, Cursor, Grok CLI, and others) can read, edit, search, and compile the project you have open, using the same tools and the same approval prompts as OpenLeaf's built-in assistant. You do not need an API key in OpenLeaf for this; the external app brings its own model.

This is useful when you already have a Claude (or similar) subscription and want that chat app to drive OpenLeaf, without pasting an API key into Settings.
