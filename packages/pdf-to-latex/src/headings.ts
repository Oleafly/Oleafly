...
export function classifyHeadings(paras: Para[], sensitivity = 0.5): Map<Para, HeadingLevel> {
  ...
  const levelOf = new Map(sizes.slice(0, 3).map((s, i) => [s, (i + 1) as HeadingLevel]));
  const out = new Map<Para, HeadingLevel>();
  for (const p of paras) {
