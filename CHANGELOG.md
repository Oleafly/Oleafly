...
- Updates finish downloading and verifying before the main window saves your
  ...
  agents remain available after shutdown finishes. The built-in assistant also
  accepts new requests after a failed update. Background shutdown has its own
  time limit, so it does not use up the time reserved for saving.
...
- A large paste into a busy terminal no longer blocks the app. Pending input is
  ...
  and Linux, including when a paste is still being written. Oversized pastes
  explain the 4 MiB limit before sending any text to the shell.
- Windows terminals can start while hidden and close without waiting for a
  cursor-position reply from the interface.
- Drawings generated by 0.2.6 remain editable. Their source is upgraded only
  when you edit them; customized source stays in a read-only preview.
...
- Files in unsupported text encodings are kept out of editable buffers. Opening
  ...
  the damaged text; the message explains how to prepare a UTF-8 copy. Citation
  imports stop when an existing bibliography or document cannot be read.
- Checkpoints preserve decomposed Unicode filenames used on macOS. Unreadable
  ...
  a checkpoint that silently omits them. If a file disappears during capture,
  Oleafly retries the complete capture once. Normal cancellation stays quiet.
...
- Restored the visible, interactive Source editor in packaged desktop builds.
  Startup splash styles are now external so Tauri's CSP rewriting does not
  block CodeMirror's runtime stylesheet; the editor also measures during its
  first layout and fold/search icons have bounded geometry.
