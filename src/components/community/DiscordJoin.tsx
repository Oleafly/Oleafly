...
export function DiscordBrandIcon({ className }: Readonly<{ className?: string }>) {
  return (
...
export function XBrandIcon({ className }: Readonly<{ className?: string }>) {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" className={className} fill="currentColor">
...
/** "12 online" with Discord's presence dot. Renders nothing until a count is known. */
export function DiscordOnlineCount({
  online,
  className,
}: Readonly<{ online: number | null; className?: string }>) {
  const { t } = useTranslation(["shell"]);
  if (online === null) return null;
  const count = formatNumber(online);
  return (
    <span
      role="img"
      data-testid="discord-online-count"
      aria-label={t(($) => $.shell.community.discordOnlineAria, { online: count })}
