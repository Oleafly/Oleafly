...
export function GeneratorsToolView() {
  ...
  const generatorCopy = {
    abstract: {
      name: t(($) => $.researchTools.generators.tasks.abstract.name),
      description: t(($) => $.researchTools.generators.tasks.abstract.description),
    },
    summarize: {
      name: t(($) => $.researchTools.generators.tasks.summarize.name),
      description: t(($) => $.researchTools.generators.tasks.summarize.description),
    },
    paraphrase: {
      name: t(($) => $.researchTools.generators.tasks.paraphrase.name),
      description: t(($) => $.researchTools.generators.tasks.paraphrase.description),
    },
    thesis: {
      name: t(($) => $.researchTools.generators.tasks.thesis.name),
      description: t(($) => $.researchTools.generators.tasks.thesis.description),
    },
  } as const;
  const activeCopy = generatorCopy[generator.id as keyof typeof generatorCopy];
  const usesSelection = generator.id === "paraphrase" || (generator.id === "summarize" && sourceMode === "selection");
  const missingSource = !projectId
    ? t(($) => $.researchTools.generators.openProject)
    : usesSelection && !source.selection
      ? t(($) => $.researchTools.generators.selectPassage)
      : null;
  ...
  return (
    <ToolPageShell
      page="generators"
      title={t(($) => $.researchTools.generators.title)}
      subtitle={t(($) => $.researchTools.generators.subtitle)}
      icon={ListChecks}
    ...
    >
      <ToolSplitView storageId="writing-generators">
        <ToolPane title={t(($) => $.researchTools.generators.taskPane)} badge={t(($) => $.researchTools.generators.taskCount, { count: GENERATORS.length })} footer={
          <p className="text-xs text-muted-foreground">{t(($) => $.researchTools.generators.taskFooter)}</p>
        }>
          <fieldset disabled={busy} className="flex min-w-0 flex-col gap-6 p-4">
            <legend className="sr-only">{t(($) => $.researchTools.generators.taskLegend)}</legend>
              ...
              {GENERATORS.map((item) => (
                ...
                >
                  ...
                  <span className="min-w-0">
                    <span className="block text-sm font-medium">{generatorCopy[item.id as keyof typeof generatorCopy].name}</span>
                    <span className="mt-1 block text-xs leading-relaxed text-muted-foreground">{generatorCopy[item.id as keyof typeof generatorCopy].description}</span>
                  </span>
                </button>
              ))}
            </div>
            <div className="space-y-3 border-t pt-4">
              <div className="flex items-center justify-between gap-3">
                <span className="text-xs font-semibold tracking-wide text-muted-foreground">{t(($) => $.researchTools.generators.source)}</span>
                {generator.id === "summarize" ? (
                  <ToolSegmentedControl
                    label={t(($) => $.researchTools.generators.summarySource)}
                    value={sourceMode}
                    options={[
                      { value: "document", label: t(($) => $.researchTools.generators.document), testId: "generator-source-document" },
                      { value: "selection", label: t(($) => $.researchTools.generators.selection), testId: "generator-source-selection" },
                    ]}
            ...
            <div className="space-y-2 border-t pt-4">
              <label htmlFor="generator-instructions" className="text-xs font-semibold tracking-wide text-muted-foreground">{t(($) => $.researchTools.generators.instructions)}</label>
              <textarea
        ...
        <ToolPane title={t(($) => $.researchTools.generators.previewPane)} badge={activeCopy.name} footer={
          <p className="text-xs text-muted-foreground">{t(($) => $.researchTools.generators.previewFooter)}</p>
        }>
