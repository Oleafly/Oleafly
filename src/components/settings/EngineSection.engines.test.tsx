...
describe("EngineSection Markdown tab", () => {
  it("reports a detected pandoc", async () => {
    ...
    expect(
      await screen.findByText(
        "Ready. Markdown projects convert with the Pandoc and Tectonic copies included with Oleafly.",
      ),
    ).toBeInTheDocument();
  ...
  it("installs a missing pandoc", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Repair Pandoc" }));
    expect(
      await screen.findByText("Repairing Pandoc with the pinned release…"),
    ).toBeInTheDocument();
