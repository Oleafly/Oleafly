...
// Completion scenarios ported from LaTeX Workshop's test/units/05_completion
...
describe("LaTeX Workshop completion scenarios", () => {
  ...
  it("hides deprecated commands the corpus flags as unusual", () => {
    const result = complete("\\usepackage{siunitx}\nx \\S");
    expect(option(result, "SI")).toBeFalsy();
  });
