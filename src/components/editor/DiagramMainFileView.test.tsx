...
it("opens drawings generated by 0.2.6 and upgrades them only after an edit", async () => {
  await open(earlySource);
  expect(mocks.readOnly).toBe(false);
  expect(mocks.write).not.toHaveBeenCalled();
  const next = structuredClone(currentModel());
  next.nodes[1].x += 40;
  act(() => changeModel(next));
  await useFilesStore.getState().flushForQuit();
  expect(useFilesStore.getState().files["main.tex"].content).toBe(standaloneDiagramSource(next));
});

it("keeps customized 0.2.6 source read-only", async () => {
  const source = earlySource.replace("\\begin{document}", "\\usepackage{amsmath}\n\\begin{document}");
  await open(source);
  expect(mocks.readOnly).toBe(true);
  await useFilesStore.getState().flushForQuit();
  expect(mocks.write).not.toHaveBeenCalled();
