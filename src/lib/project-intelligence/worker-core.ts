...
import {
  assembleProjectIntelligenceResult,
...
import {
  ...
  type ProjectIntelligenceWorkerErrorCode,
...
interface RetainedResult {
  ...
  readonly bibliographyDetails: ReadonlyMap<string, BibliographyEntryDetail>;
}

export interface ProjectIntelligenceWorkerScope {
  postMessage(message: ProjectIntelligenceWorkerResponse): void;
  close(): void;
}

export interface ProjectIntelligenceWorkerHost
  extends ProjectIntelligenceWorkerScope {
  addEventListener(
    type: "message",
...
function errorResponse(
  request: AnalyzeProjectIntelligenceRequest | BibliographyEntriesRequest,
  code: ProjectIntelligenceWorkerErrorCode,
  message: string,
  retryable: boolean,
...
export function createProjectIntelligenceWorker(
  ...
  let retained: RetainedResult | null = null;
  ...
  const analyze = (
    ...
    if (projectChanged) retained = null;
    if (
    ...
    retained = {
      ...
      bibliographyDetails: new Map(
        assembled.bibliographyDetails.map((entry) => [entry.id, entry]),
      ),
    };
    return {
      protocolVersion: PROJECT_INTELLIGENCE_PROTOCOL_VERSION,
  ...
  const bibliographyEntries = (
    request: BibliographyEntriesRequest,
  ): ProjectIntelligenceWorkerResponse => {
    if (
      !retained ||
      !sameProjectIntelligenceIdentity(retained.identity, request.identity)
    ) {
      return errorResponse(
        request,
        "stale_snapshot",
        "The requested bibliography belongs to a project revision the worker no longer holds.",
        false,
      );
    }
    const details = retained.bibliographyDetails;
  ...
  return (data: unknown) => {
    if (!isProjectIntelligenceWorkerRequest(data)) {
      if (
        typeof data === "object" &&
        data !== null &&
        "requestId" in data &&
        Number.isSafeInteger(data.requestId) &&
        (data.requestId as number) > 0 &&
        "identity" in data &&
        isProjectIntelligenceIdentity(data.identity)
      ) {
        scope.postMessage({
          protocolVersion: PROJECT_INTELLIGENCE_PROTOCOL_VERSION,
          type: "error",
    ...
    if (data.type === "bibliography-entries") {
      scope.postMessage(bibliographyEntries(data));
      return;
    }
    try {
      scope.postMessage(analyze(data));
    } catch {
      scope.postMessage(
        errorResponse(
          data,
...
export function installProjectIntelligenceWorker(
  host: ProjectIntelligenceWorkerHost,
): void {
  const handle = createProjectIntelligenceWorker(host);
