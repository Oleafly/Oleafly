...
![The searchable in-app keyboard shortcuts reference](/Oleafly/media/keyboard-shortcuts.png)
...
| ⌘L | Ctrl+L | Ask AI to edit the selection ([inline edit](/Oleafly/ai-inline-edit/)) |
...
While an [inline AI edit](/Oleafly/ai-inline-edit/) is showing its diff: **Enter** accepts, **Esc** rejects.
...
Toggle **Vim mode** in Settings, General (or via the ⌘K palette) for the full Vim keybinding set (`hjkl`, `i`, `:w`, `dd`, `ciw`, visual mode) alongside everything above. See [The editor](/Oleafly/editor/#vim-mode).

## Where else commands live

Almost everything with a shortcut also lives somewhere clickable: the [editor toolbar](/Oleafly/editor/#the-toolbar), the right-click menu, the **⌘K** command palette (fuzzy-searches every action: recompile, word count, add citation, insert figure, toggle theme, and more), and the **⌘⇧F** omnibar with its `/` scopes (`/create`, `/projects`, `/docs`, `/theme`, `/settings`).
