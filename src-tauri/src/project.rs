...
fn decode_editable_text(bytes: Vec<u8>) -> Result<String, String> {
    String::from_utf8(bytes).map_err(|_| {
        "This file uses an unsupported text encoding. Save a copy as UTF-8 before editing it in Oleafly. The original file has not been changed.".into()
    })
}
...
mod bounded_read_tests {
    ...
    fn legacy_encoded_text_cannot_enter_an_editable_buffer_lossily() {
        ...
        assert!(read_utf8_limited(&path, 1024)
            .unwrap_err()
            .contains("UTF-8"));
        assert!(super::decode_editable_text(std::fs::read(&path).unwrap()).is_err());
        assert_eq!(std::fs::read(&path).unwrap(), b"author = {M\xFCller}");
        let _ = std::fs::remove_file(path);
    }
}
...
fn canonical_supported_pandoc(candidate: &Path) -> Option<PathBuf> {
...
fn find_pandoc_on_path(path: Option<&std::ffi::OsStr>) -> Option<PathBuf> {
    let path = path?;
    ...
    std::env::split_paths(path)
        ...
        .find_map(|candidate| canonical_supported_pandoc(&candidate))
}
...
pub(crate) fn find_pandoc() -> Option<String> {
    ...
    if let Some(cached) = candidates.pop() {
        if let Some(cached) = canonical_supported_pandoc(&cached) {
            return cached.into_os_string().into_string().ok();
        }
    }
    if let Some(pandoc) = find_pandoc_on_path(std::env::var_os("PATH").as_deref()) {
        return pandoc.into_os_string().into_string().ok();
    }
    candidates.extend([
...
mod tests {
    ...
    fn pandoc_path_resolution_returns_canonical_supported_executable() {
        ...
        let executable = bin.join("pandoc");
        std::fs::write(&executable, "#!/bin/sh\nprintf 'pandoc 3.9.0.2\\n'\n").unwrap();
    ...
    async fn duplicate_project_records_the_fork_source() {
        ...
        let source_policy: oleafly_core::CheckpointPolicy =
            serde_json::from_value(serde_json::json!({
                "mode": "engine_dependencies",
                "legacy_always_include": ["source-assets/*.png"]
            }))
            .unwrap();
        super::with_project_metadata(&source_id, || {
            let mut meta = read_meta(&source_id)?;
            meta.checkpoints = source_policy.clone();
            super::write_meta(&source_id, &meta)?;
            Ok(meta)
        })
        .unwrap();
