//! Thread rollouts: one append-only JSONL file per thread under
//! `<root>/sessions/<yyyy>/<mm>/<dd>/rollout-<thread_id>.jsonl`. Each line is
//! one completed turn record (items included), so a thread replays by
//! reading the file front to back. Rollback truncates the tail; forks copy a
//! prefix into a new rollout; archived threads move out of the active tree.
