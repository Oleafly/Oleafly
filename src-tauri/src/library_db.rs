...
pub fn resync_thread(
    ...
    let existing = tx
        .query_row(
            "SELECT created_at_ms, archived FROM threads WHERE thread_id = ?1",
            [thread_id],
        ...
        .or_else(|error| match error {
            rusqlite::Error::QueryReturnedNoRows => Ok(None),
            other => Err(format!("thread metadata read failed: {other}")),
        })?;
    ...
    let (created_at_ms, archived) = existing.unwrap_or((updated_at_ms, 0));
    tx.execute(
...
mod tests {
    ...
    #[test]
    fn thread_resync_preserves_creation_time_and_archive_state() {
        let root = temp_root("thread-resync-metadata");
        let turns = vec![turn("turn-1", "first", 10, 5)];
        resync_thread(&root, "thread-1", "proj", &turns).unwrap();
        let created_at_ms = 123_i64;
        open(&root)
            .unwrap()
            .execute(
                "UPDATE threads SET created_at_ms = ?1 WHERE thread_id = 'thread-1'",
                [created_at_ms],
            )
            .unwrap();
        archive_thread(&root, "thread-1").unwrap();

        resync_thread(
            &root,
            "thread-1",
            "proj",
            &[turn("turn-2", "second", 20, 10)],
        )
        .unwrap();

        let (resynced_created_at_ms, archived): (i64, i64) = open(&root)
            .unwrap()
            .query_row(
                "SELECT created_at_ms, archived FROM threads WHERE thread_id = 'thread-1'",
                [],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
            .unwrap();
        assert_eq!(resynced_created_at_ms, created_at_ms);
