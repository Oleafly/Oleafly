...
The image-generation scripts referenced above (generate_schematic.py, generate_schematic_ai.py, generate_slide_image.py, generate_slide_image_ai.py) are not bundled with this skill. They call OpenRouter and read .env files outside the project, which does not fit Oleafly's sandboxed agent runtime. Use Oleafly's own figure tools to produce diagrams and slide images instead.
